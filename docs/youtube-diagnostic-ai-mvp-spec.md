...
Claude APIに「プロの動画プロデューサー」ペルソナを与え、入力された動画メタデータをもとに6軸診断スコアを算出する。YouTube上の実データは参照せず、Claude APIの知識と設計されたプロンプトだけで動作する。シードデータ・学習インフラ不要のため、最小工数で即日稼働できる。
...
| サムネイル解析 | Claude Vision API | 別モデル不要 |
...
YouTube Data API        Claude API で              Vector DB               クエリ→検索→
ジャンル別              hit/miss比較分析            (Chroma DB)             プロンプト注入
hit: 100件              構造化JSON化                namespace:
miss: 100件             (ジャンルプロファイル)       "genre_profiles"
```
...
✅ diagnostic.js — 診断エンジン本体
   ├─ buildDiagnosticPrompt()  : 入力→プロンプト変換
   ├─ callClaudeAPI()          : Claude API呼び出し
   ├─ parseScores()            : JSON出力パース
   └─ formatEmbed()            : Discord Embed整形

✅ index.js への !diag コマンド追加
   └─ 入力パース → executeDiagnosticTask() 呼び出し
...
✅ ジャンル選択 UI
   └─ Discord のボタン or セレクトメニュー

[推奨実装（MVP+）]
✅ サムネイル画像の添付対応（Claude Vision API）
✅ 診断履歴の JSON 保存（data/diagnostic/history/）
✅ 診断結果の再表示コマンド（!diag-history）
```
...
基本コマンド（テキストのみ）:
!diag [タイトル] / [説明文] / [タグ]
例: !diag 【初見】ゲーム名に挑戦してみた！ / VTuberがゲームを初めてプレイ / VTuber,ゲーム実況,初見

ジャンル指定:
!diag --genre VTuber [タイトル] / [説明文] / [タグ]

サムネイル付き:
!diag [テキスト入力] （メッセージに画像を添付）
...
[基本動作]
  ...
  [ ] Claude APIを呼び出して診断結果JSONを取得できる
...
[設計方針の遵守]
  [ ] Claude API のレスポンスに「〇万回再生される」「バズる」等の断言がない
...
[信頼性]
  [ ] Claude API エラー時にリトライが1回実行される
  [ ] リトライ失敗時にユーザーへのエラーメッセージが返る
  [ ] 不正なJSON返却時に診断失敗エラーが返る（システムクラッシュしない）

[コスト管理]
  [ ] 1診断あたりの Claude API コストが ¥5 以下に収まる
  [ ] プロンプトに Prompt Caching を適用してコスト削減されている
```
