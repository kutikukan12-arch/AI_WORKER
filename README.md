...
Discord Bot のセットアップ不要。**Node.js だけ**あれば今すぐ試せます。  
**APIキー不要 / .env 設定不要 / npm install 不要** — インターネット接続も不要です。
...
```powershell
# 1. リポジトリを取得（GitHubページの「Code → Download ZIP」解凍でも可）
git clone <このリポジトリのURL>
cd AI_WORKER

# 2. 予測を実行（npm install 不要。Node.js 組み込みモジュールのみ使用）
node bot/predict-cli.js --title "【衝撃】AIが仕事を奪う未来" --subs 50000 --tags "AI,テクノロジー" --duration 600
```
...
| オプション | 説明 | 例 |
|-----------|------|-----|
| `-t / --title` | 動画タイトル（**必須**）| `"タイトル"` |
| `-s / --subs` | チャンネル登録者数 | `50000` |
| `--tags` | タグ（カンマ区切り） | `"AI,技術"` |
| `-d / --duration` | 動画長（秒） | `600` |
| `-c / --channel` | チャンネル名（表示用のみ）| `"チャンネル名"` |
| `--json` | 結果を JSON でも出力 | — |
| `-h / --help` | ヘルプ表示 | — |

> **投稿前モード:** `--views` を省略または 0 にすると、タイトル・タグ・尺・登録者数のみで予測します。  
> **投稿後モード:** `-v 8500 -l 300 --comments 40` のように実績値を加えると精度が上がります。
...
| 項目 | 内容 |
|------|------|
| **動作 OS** | Windows 専用（Claude Code CLI が Windows 上で動作している前提）|
| **Claude Code** | Anthropic API キーが必要。トークン消費あり（`!project run` 実行のたびに課金）|
| **OpenAI API** | Codex レビュー（`!codex` / `!review`）は `OPENAI_API_KEY` がないと使用不可 |
| **GitHub Push** | `GITHUB_TOKEN` がないと自動 Push は無効。手動で `git push` が必要 |
| **同時実行** | `!project run` は 1 プロジェクトのみ。並列ループは非対応 |
| **タスク実行時間** | 1 タスクのタイムアウトは 5 分。大きな実装は事前に分割が必要 |
| **コマンド実行元** | 許可チャンネル（`ALLOWED_CHANNEL_IDS`）外からのコマンドは無視される |
| **Bot 再起動** | `!restart` はプロセス再起動のみ。PC がスリープ/シャットダウンすると停止 |
| **言語** | インターフェース・ログは日本語。英語ロケールでは一部表示が崩れる可能性あり |
