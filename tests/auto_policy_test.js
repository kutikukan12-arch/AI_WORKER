...
// ─────────────────────────────────────────────────────
// Phase E-4 追加: prepareNextTask の高危険度ブロック誤判定修正の確認
// NON_CHANGING_TYPES（REVIEW/RESEARCH/DOCS）は token/auth 等が含まれても
// prepareNextTask レベルでブロックされないことを index.js のソースで確認する
// ─────────────────────────────────────────────────────
console.log('\n[prepareNextTask 高危険度ブロック: 非変更系タイプの除外]');
...
test('39. prepareNextTask で taskType を assessDanger より先に確定している', () => {
  const taskTypeIdx  = prepareBody.indexOf('const taskType');
  const preDangerIdx = prepareBody.indexOf('const preDanger');
  assert.ok(taskTypeIdx >= 0, 'taskType 確定コードが見つからない');
  assert.ok(preDangerIdx >= 0, 'preDanger コードが見つからない');
  assert.ok(taskTypeIdx < preDangerIdx, 'taskType が preDanger より前に来ていない');
});

test('40. prepareNextTask に NON_CHANGING_TYPES の除外条件がある', () =>
  assert.ok(prepareBody.includes('NON_CHANGING_TYPES'), 'NON_CHANGING_TYPES が見つからない'));

test('41. REVIEW / RESEARCH / DOCS が NON_CHANGING_TYPES に含まれている', () => {
  assert.ok(prepareBody.includes("'REVIEW'"), 'REVIEW が含まれていない');
  assert.ok(prepareBody.includes("'RESEARCH'"), 'RESEARCH が含まれていない');
  assert.ok(prepareBody.includes("'DOCS'"), 'DOCS が含まれていない');
});

test('42. !NON_CHANGING_TYPES.has(...) で高危険度ブロックを条件付きにしている', () =>
  assert.ok(prepareBody.includes('NON_CHANGING_TYPES.has'), '条件が見つからない'));

test('43. security.checkPrompt は NON_CHANGING_TYPES 条件より前にある（最優先）', () => {
  const secIdx    = prepareBody.indexOf('security.checkPrompt');
  const nonChgIdx = prepareBody.indexOf('NON_CHANGING_TYPES');
  assert.ok(secIdx < nonChgIdx, 'security.checkPrompt が NON_CHANGING_TYPES より後にある');
});
