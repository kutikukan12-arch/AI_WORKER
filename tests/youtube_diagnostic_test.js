...
test('10a. AXIS_LABEL が新ラベルになっている', () => {
  ...
  assert.strictEqual(yd.AXIS_LABEL.uniqueness, '差別化',         'uniqueness ラベルが旧のまま（改善余地→差別化）');
});
...
test('10b. formatDiagnosticText に新6軸ラベルが含まれる', () => {
  ...
  assert.ok(text.includes('差別化'),         '差別化 ラベルがない（改善余地→差別化）');
});
...
test('11e. HTML に免責が含まれている', () => {
  const html = fs.readFileSync(WEB_HTML, 'utf8');
  assert.ok(html.includes('再生回数を予測・保証するものではありません'), 'HTMLに免責がない');
});
...
test('11f. HTML に6軸スコア表示ロジックがある', () => {
  ...
  assert.ok(html.includes('差別化'),         '差別化 ラベルがない（改善余地→差別化）');
});
...
test('13b. CLI --diagnose 出力に6軸ラベルが含まれる', () => {
  ...
  assert.ok(out.includes('差別化'),         '差別化 ラベルがない（改善余地→差別化）');
});
...
// ─────────────────────────────────────────────────────
// 15. 相沢CSレビュー対応 — ラベル変更・低スコアフォロー
// ─────────────────────────────────────────────────────
console.log('\n[15. 相沢CSレビュー対応]');

test('15a. AXIS_LABEL.uniqueness が「差別化」になっている（改善余地廃止）', () => {
  assert.strictEqual(yd.AXIS_LABEL.uniqueness, '差別化',
    `"改善余地" が残っている: ${yd.AXIS_LABEL.uniqueness}`);
});

test('15b. formatDiagnosticText に「改善余地」が含まれない（差別化へ変更済み）', () => {
  removeModel();
  const r    = yd.diagnose({ title: 'テスト！' });
  const text = yd.formatDiagnosticText(r, { title: 'テスト！' });
  assert.ok(!text.includes('改善余地'),
    `"改善余地" が残っている: ${text.slice(0, 300)}`);
  assert.ok(text.includes('差別化'),
    '6軸に "差別化" がない');
});

test('15c. 低スコア（<50）時に低スコアフォロー文が formatDiagnosticText に含まれる', () => {
  removeModel();
  // "a" は短すぎ・タグなし → スコア低い
  const r    = yd.diagnose({ title: 'a', tags: [], duration: 0 });
  const text = yd.formatDiagnosticText(r, { title: 'a' });
  if (r.totalScore < 50) {
    assert.ok(text.includes('低い点数でも問題ありません'),
      `低スコア(${r.totalScore})なのにフォロー文がない`);
    assert.ok(text.includes('失敗判定ではなく'),
      `フォロー文に「失敗判定ではなく」がない`);
  }
  // totalScore >= 50 の場合はスキップ（入力依存のため強制不可）
});

test('15d. 高スコア（>=50）時は低スコアフォロー文が含まれない', () => {
  saveDummyModel(30); // 強いダミーモデル
  const r    = yd.diagnose({
    title:    '【完全版】Python入門！3つのコツで10分マスター',
    tags:     Array(15).fill('tag'),
    duration: 600,
  });
  const text = yd.formatDiagnosticText(r, { title: '...' });
  if (r.totalScore >= 50) {
    assert.ok(!text.includes('低い点数でも問題ありません'),
      `高スコア(${r.totalScore})なのにフォロー文が出ている`);
  }
});

test('15e. タグ・説明文ヒントが常に formatDiagnosticText に含まれる', () => {
  removeModel();
  const inputs = [
    { title: 'a' },
    { title: '【完全版】Python入門！3つのコツで10分マスター', tags: Array(15).fill('tag') },
  ];
  for (const inp of inputs) {
    const r    = yd.diagnose(inp);
    const text = yd.formatDiagnosticText(r, inp);
    assert.ok(text.includes('YouTube 投稿画面'),
      `タグヒントがない (title="${inp.title.slice(0, 20)}"): ${text.slice(0, 200)}`);
  }
});

test('15f. HTML に「差別化」ラベルがある（改善余地廃止確認）', () => {
  const html = fs.readFileSync(WEB_HTML, 'utf8');
  assert.ok(html.includes('差別化'),  'HTML に "差別化" がない');
  assert.ok(!html.includes('改善余地'), 'HTML に "改善余地" が残っている');
});

test('15g. HTML に低スコアフォロー要素がある (#low-score-note)', () => {
  const html = fs.readFileSync(WEB_HTML, 'utf8');
  assert.ok(html.includes('low-score-note'),          '#low-score-note 要素がない');
  assert.ok(html.includes('低い点数でも問題ありません'), 'フォロー文が HTML にない');
  assert.ok(html.includes('失敗判定ではなく'),          '「失敗判定ではなく」が HTML にない');
});

test('15h. HTML にタグ・説明文ヒットがある (.tag-hint)', () => {
  const html = fs.readFileSync(WEB_HTML, 'utf8');
  assert.ok(html.includes('tag-hint'),     '.tag-hint 要素がない');
  assert.ok(html.includes('YouTube 投稿画面'), 'ヒントテキストが HTML にない');
});

test('15i. HTML: 低スコートフォローは JS で制御される（低スコアのみ表示）', () => {
  const html = fs.readFileSync(WEB_HTML, 'utf8');
  // showResult 内で totalScore < 50 の条件分岐がある
  assert.ok(html.includes('totalScore < 50'), 'JS に低スコート条件分岐がない');
  // 初期状態は display:none
  assert.ok(html.includes('low-score-note') && html.includes('display:none'),
    '#low-score-note の初期 display:none がない');
});

// ─────────────────────────────────────────────────────
// [16. Phase 2] フィードバック・利用ログ
// ─────────────────────────────────────────────────────
console.log('\n[16. Phase 2: フィードバック・利用ログ]');

test('16a. HTML に feedback-box 要素がある', () => {
  const html = fs.readFileSync(WEB_HTML, 'utf8');
  assert.ok(html.includes('feedback-box'), 'feedback-box 要素がない');
  assert.ok(html.includes('star-btn'),     'star-btn がない');
  assert.ok(html.includes('feedback-sent'), 'feedback-sent 要素がない');
});

test('16b. HTML に /feedback への fetch がある', () => {
  const html = fs.readFileSync(WEB_HTML, 'utf8');
  assert.ok(html.includes('/feedback'), 'フィードバック送信パスがない');
});

test('16c. サーバーに /feedback POST ルートがある', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'web', 'youtube-diagnostic-server.js'), 'utf8');
  assert.ok(src.includes("req.url === '/feedback'"), '/feedback ルートがない');
});

test('16d. サーバーに appendLog があり /diagnose でも呼ぶ', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'web', 'youtube-diagnostic-server.js'), 'utf8');
  assert.ok(src.includes('appendLog'),           'appendLog 関数がない');
  assert.ok(src.includes("event: 'diagnose'"),   'diagnose ログ記録がない');
  assert.ok(src.includes("event: 'feedback'"),   'feedback ログ記録がない');
});

test('16e. diagnose 結果に ok/totalScore/scores/improvements が揃っている', () => {
  const yd = require('../bot/utils/youtube-diagnostic');
  const r  = yd.diagnose({ title: '【テスト】診断テスト動画！10分で分かる', genre: 'education', duration: 600 });
  assert.ok(r.ok,             'ok が false');
  assert.ok(typeof r.totalScore === 'number', 'totalScore が数値でない');
  assert.ok(r.scores && typeof r.scores.ctr === 'number', '6軸 scores がない');
  assert.ok(Array.isArray(r.improvements),    'improvements が配列でない');
});
