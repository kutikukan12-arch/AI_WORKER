...
// ─────────────────────────────────────────────────────
// 8y. M1/M2/M3/M4 追加テスト
// ─────────────────────────────────────────────────────
console.log('\n[8y. M1/M2/M3/M4 追加テスト]');

test('M1(B案). securityBlockCount は常に 0（dead code 除去）', () => {
  // B案: securityBlocked フィールドは永続化されないため常に 0
  const ind = qg.gatherIndicators('youtube予測ai');
  assert.strictEqual(ind.securityBlockCount, 0,
    'securityBlockCount が 0 でない — フィールド未永続化なのに値が入っている');
});

test('M1(B案). assessQuality で securityBlock トリガが発火しない', () => {
  // securityBlockCount=0 なので RED トリガに含まれない
  const assessment = qg.assessQuality('youtube予測ai');
  const hasSecTrigger = assessment.redTriggers.some(t => t.includes('セキュリティブロック'));
  assert.ok(!hasSecTrigger, 'securityBlock RED トリガが発火している');
});

test('M2. index.js PRE-RUN がフェイルクローズになっている', () => {
  const src = require('fs').readFileSync(
    require('path').join(__dirname, '..', 'bot', 'index.js'), 'utf8'
  );
  const runFnStart = src.indexOf('async function handleProjectRun');
  const runFnEnd   = src.indexOf('\nasync function handle', runFnStart + 1);
  const runBody    = src.slice(runFnStart, runFnEnd > 0 ? runFnEnd : runFnStart + 8000);
  // フェイルクローズ: エラーメッセージが含まれる
  assert.ok(runBody.includes('品質ゲート評価に失敗'),
    'フェイルクローズメッセージがない');
  // qaErr catch ブロックに return が含まれる（全 runBody で確認）
  assert.ok(runBody.includes('qaErr') && runBody.includes('return'),
    'catch(qaErr) ブロックに return がない（フェイルオープン）');
  // quality gate の catch に「無視して続行」がないこと（staffing catch とは別）
  const qaErrIdx  = runBody.indexOf('qaErr');
  const qaErrBody = runBody.slice(qaErrIdx, qaErrIdx + 400);
  assert.ok(!qaErrBody.includes('無視して続行'),
    'quality gate catch にまだ「無視して続行」がある（フェイルオープン）');
});

test('M3. projectId 不明の Codex result は除外される（!taskPid）', () => {
  // _resolveTaskProject が null を返したとき continue でスキップされる
  const src = require('fs').readFileSync(
    require('path').join(__dirname, '..', 'bot', 'utils', 'quality-gate.js'), 'utf8'
  );
  // "!taskPid || taskPid !== projectId" が含まれていること
  assert.ok(src.includes('!taskPid || taskPid !== projectId'),
    'M3: null taskPid がスキップされていない');
});

test('M3. 存在しないプロジェクトへの Codex result 混入なし', () => {
  // nonexistent-project-xyz は存在しないため codexHighCount=0 のはず
  const ind = qg.gatherIndicators('nonexistent-project-xyz-abc');
  assert.strictEqual(ind.codexHighCount, 0,
    'nonexistent project に Codex 高が混入した');
});

test('M4. _isValidationFailure が stateHistory 全体をスキャンする', () => {
  // 最後のエントリが REVIEWING でなくても、途中に REVIEWING+未完了 があれば検出
  const mockTask = {
    state: 'レビュー待ち',
    stateHistory: [
      { state: '未着手',     note: '作成' },
      { state: '作業中',     note: 'Claude Code 実行開始' },
      { state: 'レビュー待ち', note: '未完了 — 変更なし' },  // validator 失敗
      { state: 'レビュー待ち', note: 'AIレビュー: 修正推奨' }, // 最後は AIレビュー
    ],
  };
  // 旧実装（最後だけ）: '修正推奨' → false（見落とし）
  // 新実装（全体）: REVIEWING+未完了 を検出 → true
  const src = require('fs').readFileSync(
    require('path').join(__dirname, '..', 'bot', 'utils', 'quality-gate.js'), 'utf8'
  );
...
test('M4. 末尾でない REVIEWING+未完了 で validator 失敗が検出される', () => {
  // gatherIndicators を使って validationFailedCount をチェックするのは難しいため
  // _isValidationFailure と同等のロジックを直接検証
  ...
  const taskWithMidFail = {
    stateHistory: [
      { state: 'レビュー待ち', note: '未完了 — 変更なし' },
      { state: 'レビュー待ち', note: 'AIレビュー: 問題なし' }, // 最後は正常
    ],
  };
  assert.ok(mockIsValidationFailure(taskWithMidFail),
    '途中の validator 失敗が検出されなかった');
  const normalTask = {
    stateHistory: [
      { state: 'レビュー待ち', note: 'AIレビュー: 問題なし' },
    ],
  };
  assert.ok(!mockIsValidationFailure(normalTask),
    '正常なレビュー待ちが誤って validator 失敗と判定された');
});
