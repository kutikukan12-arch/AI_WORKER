...
test('F-9. applyStaffingPlan: apply 直前に busy 化したワーカーは skipped になる（H1）', () => {
  // idle ワーカーを登録して toRemove 候補にする
  wr.addWorker('IMPLEMENTER', 'f9-impl', 'assign-test');
  // apply 直前に busy にする（race condition シミュレーション）
  wr.updateWorkerStatus('f9-impl', wr.WORKER_STATUS.BUSY, 'task_race');
  ...
  // computeDelta 時点で busy → toRemove に入らないはず
  const delta = cm.computeDelta('assign-test', plan);
  assert.ok(!delta.toRemove.includes('f9-impl'),
    'busy なのに toRemove に含まれた (computeDelta で除外されるべき)');
  console.log('  ℹ️  F-9: toRemove=' + JSON.stringify(delta.toRemove));

  wr.updateWorkerStatus('f9-impl', wr.WORKER_STATUS.IDLE, null);
  wr.removeWorker('f9-impl');
});

test('F-10. applyStaffingPlan: removeWorker が BUSY を返したら skipped に入る', () => {
  // 手動で toRemove に入るシナリオを作るため、idle で入れてから直前に busy 化
  wr.addWorker('IMPLEMENTER', 'f10-impl', 'assign-test');
  // computeDelta は idle で除外 → toRemove に入る
  const plan = {
    workers: { IMPLEMENTER: 0, REVIEWER: 0, TESTER: 0, RESEARCHER: 0 },
    total: 0,
    scaleLabel: '極小',
  ...
  const delta = cm.computeDelta('assign-test', plan);
  // f10-impl が toRemove に含まれていることを確認
  if (!delta.toRemove.includes('f10-impl')) {
    // toRemove に入っていない場合は優先度等の問題でスキップ
    info('F-10: f10-impl が toRemove に含まれなかった → テストスキップ');
    wr.removeWorker('f10-impl');
    return;
  }
  // applyStaffingPlan 実行直前に busy 化（race condition）
  wr.updateWorkerStatus('f10-impl', wr.WORKER_STATUS.BUSY, 'task_race2');
  // 直接 applyStaffingPlan の削除部分をシミュレート
  const res = wr.removeWorker('f10-impl'); // force:false → BUSY で拒否
  assert.strictEqual(res.ok, false, 'busy なのに削除できた');
  assert.strictEqual(res.reason, 'BUSY');
  console.log('  ℹ️  F-10: BUSY guard ok: reason=' + res.reason);
  wr.updateWorkerStatus('f10-impl', wr.WORKER_STATUS.IDLE, null);
  wr.removeWorker('f10-impl');
});

test('F-11. computeDelta: toAdd の累積を考慮して MAX_WORKERS を正しく計算する（H3）', () => {
  // MAX_WORKERS = 4 (worker-registry.js のデフォルト)
  // 現在 0 ワーカー、plan が IMPLEMENTER:3 + REVIEWER:1 = 4
  // → toAdd はちょうど MAX_WORKERS 以内
  const plan = {
    workers: { IMPLEMENTER: 2, REVIEWER: 1, TESTER: 1, RESEARCHER: 0 },
    total: 4,
    scaleLabel: '中規模',
  };
  const delta = cm.computeDelta('assign-test', plan);
  const totalAdded = delta.toAdd.reduce((s, a) => s + a.count, 0);
  const wrMax      = wr.MAX_WORKERS;
  const current    = wr.listWorkers().filter(w => w.projectId === 'assign-test').length;
  assert.ok(totalAdded + current <= wrMax,
    `toAdd 累積 ${totalAdded} + 現在 ${current} が MAX_WORKERS(${wrMax}) を超えた`
  );
  console.log(`  ℹ️  F-11: totalAdded=${totalAdded} MAX_WORKERS=${wrMax} warnings=${delta.warnings.join('|')}`);
});
